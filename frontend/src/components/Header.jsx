...
const Header = ({
  ...
  const { sections: statisticsSections } = useGenderStatisticsSections();
  const sections = statisticsSections.map((item) => ({
    ge: item.ge,
    en: item.en,
    route: `/statistics/${item.id}`,
  }));
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              <span
                className={`block w-6 h-[2px] bg-gray-700 transition-transform duration-300 ${menuOpen ? "rotate-45 translate-y-[7px]" : ""}`}
              />
              <span
                className={`block w-6 h-[2px] bg-gray-700 transition-opacity duration-300 ${menuOpen ? "opacity-0" : ""}`}
              />
              <span
                className={`block w-6 h-[2px] bg-gray-700 transition-transform duration-300 ${menuOpen ? "-rotate-45 translate-y-[-7px]" : ""}`}
              />
              ...
              idx === 0 ? (
                <div key={item}>
                  ...
                  >
                    {item}
                    <svg
                      className={`w-3 h-3 transition-transform ${mobileStatsOpen ? "rotate-180" : ""}`}
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
